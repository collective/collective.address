...
class CountryVocabulary:
    ...
    def __call__(self, context, query=None):
        ...
        items = [
            (
                it.numeric,
                plone.api.portal.translate(it.name, domain="iso3166", lang=lang),
            )
        ...
        items = [
            ...
            for it in items
            if query is None or query in it[1].lower()
        ]
...
def get_pycountry_name(country_id):
    ...
    return plone.api.portal.translate(
        country.name, domain="iso3166", lang=plone.api.portal.get_current_language()
    )
